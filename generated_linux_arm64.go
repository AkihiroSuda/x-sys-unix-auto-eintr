...
// PtraceGetRegSetArm64 is an alias of golang.org/x/sys/unix.PtraceGetRegSetArm64, wrapped to automatically retry on EINTR.
func PtraceGetRegSetArm64(pid, addr int, regsout *unix.PtraceRegsArm64) error {
	var (
		_v0 error
	)
	for {
		_v0 = unix.PtraceGetRegSetArm64(pid, addr, regsout)
		if _v0 != syscall.EINTR {
			break
		}
	}
...
// PtraceSetRegSetArm64 is an alias of golang.org/x/sys/unix.PtraceSetRegSetArm64, wrapped to automatically retry on EINTR.
func PtraceSetRegSetArm64(pid, addr int, regs *unix.PtraceRegsArm64) error {
	var (
		_v0 error
	)
	for {
		_v0 = unix.PtraceSetRegSetArm64(pid, addr, regs)
		if _v0 != syscall.EINTR {
			break
		}
	}
