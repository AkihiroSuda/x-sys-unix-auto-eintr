...
// IoctlGetPtmget is an alias of golang.org/x/sys/unix.IoctlGetPtmget, wrapped to automatically retry on EINTR.
func IoctlGetPtmget(fd int, req uint) (*unix.Ptmget, error) {
	...
	for {
		_v0, _v1 = unix.IoctlGetPtmget(fd, req)
		if _v1 != syscall.EINTR {
			break
		}
	}
...
func Fstatvfs(fd int, buf *unix.Statvfs_t) (err error) {
	for {
		err = unix.Fstatvfs(fd, buf)
		if err != syscall.EINTR {
			break
		}
	}
...
func Statvfs(path string, buf *unix.Statvfs_t) (err error) {
	for {
		err = unix.Statvfs(path, buf)
		if err != syscall.EINTR {
			break
		}
	}
...
// Fstatvfs1 is an alias of golang.org/x/sys/unix.Fstatvfs1, wrapped to automatically retry on EINTR.
func Fstatvfs1(fd int, buf *unix.Statvfs_t, flags int) (err error) {
	for {
		err = unix.Fstatvfs1(fd, buf, flags)
		if err != syscall.EINTR {
			break
		}
	}
...
// Statvfs1 is an alias of golang.org/x/sys/unix.Statvfs1, wrapped to automatically retry on EINTR.
func Statvfs1(path string, buf *unix.Statvfs_t, flags int) (err error) {
	for {
		err = unix.Statvfs1(path, buf, flags)
		if err != syscall.EINTR {
			break
		}
	}
