...
func Fstat(fd int, s *unix.Stat_t) (err error) {
	for {
		err = unix.Fstat(fd, s)
		if err != syscall.EINTR {
			break
...
func Fstatat(dirfd int, path string, s *unix.Stat_t, flags int) (err error) {
	for {
		err = unix.Fstatat(dirfd, path, s, flags)
		if err != syscall.EINTR {
			break
		}
	}
...
func Lstat(path string, s *unix.Stat_t) (err error) {
	for {
...
func Stat(path string, s *unix.Stat_t) (err error) {
	for {
...
// PtraceGetRegsMipsle is an alias of golang.org/x/sys/unix.PtraceGetRegsMipsle, wrapped to automatically retry on EINTR.
func PtraceGetRegsMipsle(pid int, regsout *unix.PtraceRegsMipsle) error {
	var (
		_v0 error
	)
	for {
		_v0 = unix.PtraceGetRegsMipsle(pid, regsout)
		if _v0 != syscall.EINTR {
			break
		}
	}
...
// PtraceSetRegsMipsle is an alias of golang.org/x/sys/unix.PtraceSetRegsMipsle, wrapped to automatically retry on EINTR.
func PtraceSetRegsMipsle(pid int, regs *unix.PtraceRegsMipsle) error {
	var (
		_v0 error
	)
	for {
		_v0 = unix.PtraceSetRegsMipsle(pid, regs)
		if _v0 != syscall.EINTR {
			break
		}
	}
...
// PtraceGetRegsMips64le is an alias of golang.org/x/sys/unix.PtraceGetRegsMips64le, wrapped to automatically retry on EINTR.
func PtraceGetRegsMips64le(pid int, regsout *unix.PtraceRegsMips64le) error {
	var (
		_v0 error
	)
	for {
		_v0 = unix.PtraceGetRegsMips64le(pid, regsout)
		if _v0 != syscall.EINTR {
			break
		}
	}
...
// PtraceSetRegsMips64le is an alias of golang.org/x/sys/unix.PtraceSetRegsMips64le, wrapped to automatically retry on EINTR.
func PtraceSetRegsMips64le(pid int, regs *unix.PtraceRegsMips64le) error {
	var (
		_v0 error
	)
	for {
		_v0 = unix.PtraceSetRegsMips64le(pid, regs)
		if _v0 != syscall.EINTR {
			break
		}
	}
...
func SyncFileRange(fd int, off int64, n int64, flags int) (err error) {
	for {
		err = unix.SyncFileRange(fd, off, n, flags)
		if err != syscall.EINTR {
			break
		}
	}
